...
import { Chip, Avatar, Icon } from 'sveltify/src';
import { mdiPlus, mdiMagnet, mdiAccount, mdiViewDashboard } from '@mdi/js';

const variants = ['label', 'link', 'outlined', 'pill'];

const items = [{ mdiPlus }, { mdiMagnet }, { mdiAccount }, { mdiViewDashboard }].map((mdi) => ({
  name: Object.keys(mdi)[0],
  value: Object.values(mdi)[0],
}));
...
const controls = {
  ...
  icon: {
    ...
    items,
    format: (val) =>
      items.find((i) => i.value == val) ? items.find((i) => i.value == val).name : '', // eslint-disable-line eqeqeq
  },
  size: {
    type: 'select',
